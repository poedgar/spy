  ...
  it('allows an invited agent to authenticate and join an existing operation directly', () => {
    ...
    cy.get('#invite-url-input')
      ...
      .then((inviteUrl) => {
        ...
        // The join round-trips through live Firestore (read game, write new
        // player, confirm) with retries on top, which can comfortably exceed
        // a short timeout under real CI network latency.
        cy.get('#lobby-header', { timeout: 15000 }).should('be.visible');
